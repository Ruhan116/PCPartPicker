...
class Ui_CasePage(object):
    ...
    def search_by_keyword(self):
        ...
        cursor.execute("SELECT * FROM Cases WHERE Name LIKE ? OR Size LIKE ? OR Price LIKE ? OR id LIKE ?", ('%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%'))
        rows = cursor.fetchall()
        self.populate_table(rows)
        connection.close()
