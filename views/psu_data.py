...
class Ui_PSUPage(object):
    def setupUi(self, MainWindow):
        ...
        # Title Label
        self.label = QtWidgets.QLabel("Choose A PSU", self.centralwidget)
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Name", "Size", "Wattage", "Price", "Action", "Edit"])
    ...
    def load_psu_data(self):
        """Load PSU data based on the selected sorting option."""
        ...
        if self.sort_ascending.isChecked():
            query = """
            SELECT id, Name, Size, Wattage, Price
            FROM PSU
    ...
    def search_by_keyword(self):
        ...
        if keyword:
            ...
            self.load_psu_data()  # Reload data with the current sorting option
            return
    ...
    def handle_add_button(self, row):
        ...
        print(f"'Add' button clicked for PSU Name: {psu_name}")
    ...
    def handle_edit_button(self, row):
        ...
        save_btn.clicked.connect(lambda: self.save_edit(dialog, id_, name_edit.text(), size_edit.text(), wattage_edit.text(), price_edit.text()))
