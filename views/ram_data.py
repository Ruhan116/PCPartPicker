...
class Ui_RAMPage(object):
    def setupUi(self, MainWindow):
        ...
        # Title Label
        self.label = QtWidgets.QLabel("Choose RAM", self.centralwidget)
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Name", "Size", "Type", "Bus Speed", "Quantity", "Price", "Action", "Edit"])
    ...
    def load_ram_data(self):
        """Load RAM data based on the selected sorting option."""
        ...
        if self.sort_ascending.isChecked():
            query = """
            SELECT id, Name, Size, Type, Bus_Speed, Quantity, Price
            FROM RAM
    ...
    def search_by_keyword(self):
        ...
        if keyword:
            ...
            self.load_ram_data()  # Reload data with the current sorting option
            return
    ...
    def handle_add_button(self, row):
        ...
        print(f"'Add' button clicked for RAM Name: {ram_name}")
    ...
    def handle_edit_button(self, row):
        ...
        save_btn.clicked.connect(lambda: self.save_edit(dialog, id_, name_edit.text(), size_edit.text(), type_edit.text(), bus_speed_edit.text(), quantity_edit.text(), price_edit.text()))
