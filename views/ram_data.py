...
class Ui_RAMPage(object):
    def setupUi(self, MainWindow):
        ...
        self.table.setColumnCount(8)  # 7 columns for data + 1 for "Add" button column
        self.table.setRowCount(0)
        
        # Set up table headers
        headers = ["id", "Name", "Size", "Type", "Bus_Speed", "Quantity", "Price", "Action"]
    ...
    def load_ram_data(self):
        ...
        if count_value > 0:
            cursor.execute("SELECT * FROM RAM WHERE Count <= ?", (count_value,))
        else:
    ...
    def search_by_keyword(self):
        ...
        cursor.execute("SELECT * FROM RAM WHERE id LIKE ? OR Name LIKE ? OR Size LIKE ? OR Type LIKE ? OR Bus_Speed LIKE ? OR Quantity LIKE ? OR Price LIKE ?", ('%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%', '%' + keyword + '%'))
        rows = cursor.fetchall()
        self.populate_table(rows)
        connection.close() 
